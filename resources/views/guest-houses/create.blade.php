...
<div class="py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            <h1 class="text-2xl font-bold text-gray-900 mb-6">Tambah Guest House Baru</h1>

            <form action="{{ route('guest-houses.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Nama Guest House</label>
                        <input type="text" name="nama" value="{{ old('nama') }}" required class="w-full rounded-lg border-gray-300 @error('nama') border-red-500 @enderror">
                        @error('nama')
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="3" required class="w-full rounded-lg border-gray-300 @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="5" class="w-full rounded-lg border-gray-300">{{ old('deskripsi') }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Harga per Malam (Rp)</label>
                            <input type="number" name="harga_per_malam" value="{{ old('harga_per_malam') }}" min="0" required class="w-full rounded-lg border-gray-300 @error('harga_per_malam') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="number" name="kapasitas" value="{{ old('kapasitas', 2) }}" min="1" required class="w-full rounded-lg border-gray-300">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach(['WiFi', 'AC', 'TV', 'Kamar Mandi', 'Dapur', 'Parkir', 'Kolam Renang', 'Gym', 'Laundry', 'Breakfast'] as $fasilitas)
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" name="fasilitas[]" value="{{ $fasilitas }}" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ (old('fasilitas') && in_array($fasilitas, old('fasilitas'))) ? 'checked' : '' }}>
